...
/**
 * Fetch user info from Auth + User collections
 */
const getUserInfo = async (authId: string): Promise<DecodedUser | null> => {
  let error, auth, user, data: DecodedUser;
...
/**
 * Fetch admin info
 * ⚠️ Decide: If your JWT stores `admin._id`, use findById.
 * If your Admin schema stores `authId`, use findOne({ auth: authId }).
 */
const getAdminInfo = async (id: string): Promise<DecodedAdmin | null> => {
  let error, admin, data: DecodedAdmin;
...
/**
 * Middleware factory: authorizeToken
 */
export const authorizeToken = (secret: string, isAdminCheck = false) => {
  return asyncHandler(
    async (req: Request, res: Response, next: NextFunction): Promise<any> => {
      ...
      if (isAdminCheck) {
        ...
        if (!data) {
          return next(
            createError(StatusCodes.NOT_FOUND, "Account Not Found")
          );
        }
        req.user = data;
      }
...
/**
 * Utility: verify JWT outside middleware (for sockets, etc.)
 */
export async function verifyJwtAndFetchUser(
  rawToken: string,
  secret: string
 ...
 */
export const authorize = authorizeToken(process.env.JWT_ACCESS_SECRET!);
export const admin_authorize = authorizeToken(
